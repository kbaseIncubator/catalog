...
class MongoCatalogDBI:
    ...
    # last_state is for concurency control.  If set, it will match on state as well, and will fail
    ...
    def set_module_registration_state(self, module_name='', git_url='', new_state=None,
        ...
        if new_state:
            ...
            result = self.modules.update_one(query, {
                '$set': {'state.registration': new_state, 'state.error_message': error_message}})
            return self._check_update_result(result)
        return False
    ...
    def set_module_release_state(self, module_name='', git_url='', new_state=None, last_state=None,
        ...
        if new_state:
            ...
            result = self.modules.update_one(query, {'$set': {'state.release_approval': new_state,
    ...
    def update_dev_version(self, version_info):
        if version_info:
            if 'git_commit_hash' in version_info and 'module_name_lc' in version_info:
                ...
                # try to insert
                try:
                    v_info_copy = copy.deepcopy(
                        version_info)  # copy because insert has side effects on the data if it fails, stupid mongo!
                    self.module_versions.insert_one(v_info_copy)
                # if that doesn't work, try to update (NOTE: by now this version should only be
                # updatable if it is a dev or orphan version, and  we assume that check has
                # already been made)
                except:
                ...
                result = self.modules.update_one(query, {'$set': {
                    'current_versions.dev': {'git_commit_hash': version_info['git_commit_hash']}}})
                return self._check_update_result(result)
            else:
    ...
    def save_local_function_specs(self, local_functions):
        ...
        for l in local_functions:
            ...
            # insert or update- allows us to capture the latest info if a specific commit is
            # reregistered without adding a duplicate or throwing an error
    ...
    def list_local_function_info(self, release_tag=None, module_names=[]):
        ...
        git_commit_hash_list = []
        git_mod_name_lookup = {}
        for mod in self.db.modules.find(query,
                                        {'module_name_lc': 1, 'current_versions': 1, '_id': 0}):
            if check_mods:
            ...
            for rt in ['release', 'beta', 'dev']:
                if rt in mod['current_versions'] and mod['current_versions'][rt] is not None:
                    if 'git_commit_hash' in mod['current_versions'][rt]:
                        if mod['current_versions'][rt]['git_commit_hash'] == \
                                mod['current_versions'][tag]['git_commit_hash']:
                            release_tags.append(rt)
            git_mod_name_lookup[mod['module_name_lc'] + mod['current_versions'][tag][
                'git_commit_hash']] = release_tags
        ...
        local_funcs = list(
            self.db.local_functions.find({'git_commit_hash': {'$in': git_commit_hash_list}},
                                         included_fields))
    ...
    def get_local_function_spec(self, functions):
        ...
        # for now be lazy and break up the call into separate queries and loops over mod list...
        # lots of optimization you could do here although we expect in general, most users will
        # only get module details one at a time
        for f in functions:
            query = {
            ...
            if 'release_tag' in f:
                ...
                if f['release_tag'] in module['current_versions'] and module['current_versions'][
                    f['release_tag']] is not None:
                    if 'git_commit_hash' in m['current_versions'][f['release_tag']]:
                        query['git_commit_hash'] = m['current_versions'][f['release_tag']][
                            'git_commit_hash']
                else:
                    continue
                ...
                if 'git_commit_hash' in f:
                    ...
                    for t in ['release', 'beta', 'dev']:
                        if t in module['current_versions'] and module['current_versions'][
                            t] is not None:
                            if 'git_commit_hash' in module['current_versions'][t]:
                                query['git_commit_hash'] = module['current_versions'][t][
                                    'git_commit_hash']
                                break
            ...
            # if we didn't get anything, then the git_commit_hash or function_id was wrong, and we
            #  just continue
    ...
    def set_module_name(self, git_url, module_name):
        ...
        result = self.modules.update_one(query, {
            '$set': {'module_name': module_name, 'module_name_lc': module_name.lower()}})
        return self._check_update_result(result)
    ...
    def substitute_hashes_for_version_info(self, module_list):
        ...
        for mod in module_list:
            if 'module_name_lc' not in mod:
                raise ValueError(
                    'DB Error: module_name_lc must be specified to get version documents')
        ...
        for mod in module_list:
            if 'current_versions' in mod:
                for tag in ['release', 'beta', 'dev']:
                    if tag in mod['current_versions'] and mod['current_versions'][tag] is not None:
                        if 'git_commit_hash' in mod['current_versions'][tag]:
                            mod['current_versions'][tag] = ver_lookup[mod['module_name_lc']][
                                mod['current_versions'][tag]['git_commit_hash']]
                ...
                for r in mod['release_version_list']:
                    if 'git_commit_hash' in r:
                        new_release_version_list.append(
                            ver_lookup[mod['module_name_lc']][r['git_commit_hash']])
    ...
    def list_service_module_versions_with_tag(self, tag):
        ...
        result = []
        for m in mods:
            if m['current_versions'].get(tag) is not None:
                if m['current_versions'][tag].get('dynamic_service') == 1:
                    result.append({
    ...
    def delete_module(self, module_name, git_url):
        ...
        if module_details['release_version_list']:
            raise ValueError(
                'Cannot delete module that has released versions.  Make it inactive instead.')
    ...
    def _check_update_result(self, result):
        # TODO: This is an odd function. I modded it to maintain compatability but it seems like
    ...
    def add_exec_stats_users(self, user_id, creation_time, exec_start_time,
        ...
        self.exec_stats_users.update_one(
            {'user_id': user_id, 'type': type, 'time_range': time_range},
    ...
    def update_db_2_to_3(self):
        ...
        for m in modules_collection.find({}):
            ...
            for tag in ['release', 'beta', 'dev']:
                # we can skip release tags because that is already in the release_version_list
                if m['current_versions'].get(tag) is not None:
    ...
    def update_db_3_to_4(self):
        ...
        for cg in client_groups_collection.find({}):
            if 'app_id' in cg:
                ...
                if len(tokens) != 2:
                    print(
                        '   When updating client groups, bad app_id found.  Record will be lost:')
                    pprint.pprint(cg)
                new_cg = {
