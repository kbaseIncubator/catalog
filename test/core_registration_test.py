...
class CoreRegistrationTest(unittest.TestCase):
    ...
    def test_full_module_lifecycle(self):
        ...
        timeout = 60  # seconds
        while True:
            state = self.catalog.get_module_state(self.cUtil.anonymous_ctx(), {'git_url': giturl})[
                0]
        ...
        for meth in method_list:
            if meth['id'] == module_name + '/test_method_1' and meth['namespace'] == module_name:
                methMissing = False
        self.assertTrue(methMissing,
                        'Make sure we did not find the method in NMS under the beta tag')
        # but should not appear under beta or release
        ...
        for meth in method_list:
            if meth['id'] == module_name + '/test_method_1' and meth['namespace'] == module_name:
                methMissing = False
        self.assertTrue(methMissing,
                        'Make sure we did not find the method in NMS under the release tag')
        ...
        self.catalog.review_release_request(self.cUtil.admin_ctx(),
                                            {'module_name': module_name, 'decision': 'denied',
                                             'review_message': 'ask later'})
        releases = self.catalog.list_requested_releases(self.cUtil.anonymous_ctx())[0]
        found = False
        for r in releases:
            if r['git_commit_hash'] == githash:
        ...
        state = \
        self.catalog.get_module_state(self.cUtil.anonymous_ctx(), {'module_name': module_name})[0]
        ...
        while True:
            ...
            if state['registration'] in ['complete', 'error']:
                break
            self.assertTrue(time() - start < timeout,
                            'simple registration build 2 exceeded timeout of ' + str(
                                timeout) + 's')
        self.assertEqual(state['registration'], 'complete')
        ...
        self.assertEqual(sorted(info['dev']['narrative_methods']),
        ...
        self.assertEqual(sorted(info['release']['narrative_methods']),
                         sorted(['test_method_1', 'test_method_2']))
        ...
        while True:
            ...
            if state['registration'] in ['complete', 'error']:
                break
            self.assertTrue(time() - start < timeout,
                            'simple registration build 3 exceeded timeout of ' + str(
                                timeout) + 's')
        self.assertEqual(state['registration'], 'error')
        sleep(3)  # sleep to make sure the catalog db gets the final log messages
        log = self.catalog.get_parsed_build_log(self.cUtil.anonymous_ctx(),
        ...
        self.assertTrue(found_correct_error,
                        'correct error was thrown in log for semantic version error')
        ...
        while True:
            ...
            if state['registration'] in ['complete', 'error']:
                break
            self.assertTrue(time() - start < timeout,
                            'simple registration build 4 exceeded timeout of ' + str(
                                timeout) + 's')
        self.assertEqual(state['registration'], 'complete')
        ...
        with self.assertRaises(ValueError) as e:
            self.catalog.request_release(self.cUtil.user_ctx(),
                                         {'module_name': info['module_name']})
        self.assertEqual(str(e.exception),
                         'Cannot request release - beta version has same version number to '
                         'released version.')
        ...
        while True:
            ...
            if state['registration'] in ['complete', 'error']:
                break
            self.assertTrue(time() - start < timeout,
                            'simple registration build 5 exceeded timeout of ' + str(
                                timeout) + 's')
        self.assertEqual(state['registration'], 'complete')
        ...
        with self.assertRaises(ValueError) as e:
            self.catalog.request_release(self.cUtil.user_ctx(),
                                         {'module_name': info['module_name']})
        self.assertEqual(str(e.exception),
                         'Cannot request release - beta semantic version (0.0.1) must be greater '
                         'than the released semantic version 0.0.2, as determined by http://semver.org')
    ...
    def test_module_with_bad_spec(self):
        ...
        self.assertEqual(state['registration'], 'error')
        self.assertTrue(
            'Invalid narrative method specification (test_method_2)' in state['error_message'])
    ...
    def test_active_inactive_remove_module(self):
        ...
        self.assertEqual(self.catalog.is_registered({}, {'module_name': 'CatalogTestModule2'})[0],
                         1)
        method_list = self.nms.list_methods({'tag': 'dev'})
        foundMeth = False
        for meth in method_list:
            if meth['id'] == 'CatalogTestModule2/test_method_1' and \
                    meth['namespace'] == 'CatalogTestModule2':
                foundMeth = True
        self.assertTrue(foundMeth, 'Make sure we found the method in NMS')
        ...
        foundMeth = False
        for meth in method_list:
            if meth['id'] == 'CatalogTestModule2/test_method_1' and meth[
                'namespace'] == 'CatalogTestModule2':
                foundMeth = True
        self.assertTrue(foundMeth, 'Make sure we found the method in NMS')
        ...
        self.assertEqual(self.catalog.is_registered({}, {
            'git_url': 'https://github.com/kbaseIncubator/release_history'})[0], 1)
